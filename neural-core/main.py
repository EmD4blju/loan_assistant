...
from logs.log_controller import LogController

log = getLogger(__name__)


def main():
    log_controller = LogController(config_path=Path(__file__).parent / 'conf' / 'logging_config.json', dirs=Path(__file__).parent / 'logs')
    log_controller.start()
    
    log.info("Welcome to Loan Assistant Neural Core!")
    
    #~ Load dataset
    dataset_path = Path(__file__).parent / 'data' / 'raw' / 'loan_data.csv'
